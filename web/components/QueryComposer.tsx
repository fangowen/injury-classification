    ...
    <div className="space-y-4">
      <div className="relative">
        ...
        <button
          ...
          className="btn-send absolute right-2 top-1/2 -translate-y-1/2"
        >
          {busy ? (
            <span className="h-2 w-2 animate-pulse rounded-full bg-bg/80" />
          ) : (
      ...
      {!compact && expanded && (
        <div className="flex flex-wrap gap-2">
          <input
            ...
            placeholder="Age"
            disabled={busy}
            className="rounded-full border border-border bg-surface px-3.5 py-2 text-sm outline-none transition focus:border-accent placeholder:text-subtle disabled:opacity-60 w-20"
          />
          <select
            value={sex}
            onChange={(e) => setSex(e.target.value)}
            disabled={busy}
            className="rounded-full border border-border bg-surface px-3.5 py-2 text-sm outline-none transition focus:border-accent disabled:opacity-60 w-28"
          >
          ...
          <input
            ...
            placeholder="Sport"
            disabled={busy}
            className="rounded-full border border-border bg-surface px-3.5 py-2 text-sm outline-none transition focus:border-accent placeholder:text-subtle disabled:opacity-60 flex-1 min-w-[8rem]"
          />
          <input
            value={freq}
            ...
            disabled={busy}
            className="rounded-full border border-border bg-surface px-3.5 py-2 text-sm outline-none transition focus:border-accent placeholder:text-subtle disabled:opacity-60 flex-1 min-w-[10rem]"
          />
          <input
            value={duration}
